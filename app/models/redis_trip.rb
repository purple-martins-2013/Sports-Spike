class RedisTrip < ActiveRecord::Base
  ...
  def populate_algorithm_fields
    ...
    if RedisTrip.all.count > LONG_EMA_PERIOD + SIGNAL_LINE_PERIOD
  ...
  def calculate_short_ema
    ...
    elsif RedisTrip.all.size > SHORT_EMA_PERIOD + 1
  ...
  def calculate_long_ema
    if RedisTrip.all.size == LONG_EMA_PERIOD + 1
      # self.long_ema = RedisTrip.all.pluck('tweet_count').inject(:+) / LONG_EMA_PERIOD
      self.long_ema = RedisTrip.order('created_at DESC').limit(LONG_EMA_PERIOD).pluck('tweet_count').inject(:+) / LONG_EMA_PERIOD
    elsif RedisTrip.all.size > LONG_EMA_PERIOD + 1
  ...
  def calculate_signal_line
    if RedisTrip.all.size == LONG_EMA_PERIOD + SIGNAL_LINE_PERIOD + 1
      self.signal_line = RedisTrip.order('created_at DESC').limit(SIGNAL_LINE_PERIOD).pluck('macd').inject(:+) / SIGNAL_LINE_PERIOD 
    elsif RedisTrip.all.size > LONG_EMA_PERIOD + SIGNAL_LINE_PERIOD + 1
